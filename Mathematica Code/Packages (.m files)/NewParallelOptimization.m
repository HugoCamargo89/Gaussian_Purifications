...
BeginPackage["NewParallelOptimization`",{"FundamentalDefinitions`"}]

NewParallelOptimise::usage="Optimisation of a scalar function over the manifold of Gaussian states. 8 input arguments are (1) function to be optimised f[ M, J0], (2) associated gradient function df[ M, J0, Kbasis], (3) initial J, (4) initial M, (5) specification of basis types for subsystems A and B { basisA, basisB} (must be strings, specify 'None' for none, (6) dimensions of subsystems { dimA, dimB}, (7) tolerance on norm of gradient, and (8) step limit (default=infinity)"

Begin["Private`"]

NewParallelOptimise[function_, gradfunction_, J0_, M0_, {basisA_, basisB_}, {dimA_, dimB_}, tol_, del2_:False, steplimit_:\[Infinity]]:=

	Module[{smallK, KA, KB, K, (* Lie algebra basis *)
			...
			FinalE, FinalM, FinalElist, FinalNormlist,
			G0, invG0, invmetric, metric}, (* Results *)
		...
		K=Which[
			...
			basisA=="None", smallK=KB; Table[PadLeft[Ki,{Length[J0],Length[J0]}],{Ki,KB}],
			
			(* Case 2: Only A is varied *)
			basisB=="None", smallK=KA; Table[PadRight[Ki,{Length[J0],Length[J0]}],{Ki,KA}],
			
			(* Case 3: Both A and B are varied *)
			True, smallK=Table[ArrayFlatten[{{KAi,0},{0,KBi}}],{KAi,KA},{KBi,KB}]
		];
		...
		G0=J0.\[CapitalOmega]qpqp[Length[J0]/2]; invG0=Inverse[G0];
	...
	Return[{FinalE,FinalM,CorrList//Total,FinalElist,FinalNormlist,metric}]
];
