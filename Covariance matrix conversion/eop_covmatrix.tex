...
F^2_{j,k} &= \frac{1}{2}\sandwich{\psi}{\fd_j \fe_k - \fe_j \fd_k}{\psi} \ , \\
F^3_{j,k} &= \frac{1}{2}\sandwich{\psi}{\i(\fe_j \fe_k + \fd_j \fd_k)}{\psi}  \ , &
F^4_{j,k} &= \frac{1}{2}\sandwich{\psi}{\i(\fd_j \fe_k + \fe_j \fd_k)}{\psi} \ .
\end{align}
...
\end{equation} 
where $W \in U(N)$, so that the generating matrix becomes block-diagonal. Explicitly, $A = W^\dagger \Sigma W$ with
\begin{equation}
\Sigma = 
\bigoplus_k
\begin{pmatrix}
...
\end{equation}
We can now compute the covariance matrices $\tilde{F}^k$ in the transformed operators, which becomes a two-mode problem. Solving it yields
\begin{align}
\tilde{F}^1 &= 
\bigoplus_k
\begin{pmatrix}
...
\bigoplus_k
\begin{pmatrix}
0 & \Im(\lambda_k)/(1 + |\lambda_k|^2) \\
-\Im(\lambda_k)/(1 + |\lambda_k|^2) & 0
\end{pmatrix}\ , \\
\tilde{F}^4 &= 
\bigoplus_k
\begin{pmatrix}
...
\end{pmatrix}\ .
\end{align}
To revert to the original fermionic operators, we then simply compute $F=W^\dagger \tilde{F} W$ and $G=W^\dagger \tilde{G} W$.
